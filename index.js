...
class UpstreamProxy {
  ...
  constructor({
    ...
    let server = Object.keys(this.ssl).length > 0 ? tls.createServer(this.ssl, (socket) => this._handleConnection(socket)) : net.createServer((socket) => this._handleConnection(socket))
  ...
  _handleConnection(socket) {
    ...
    socket.once('error', (err) => socket.end());
  ...
  _handleData(socket, data) {
    ...
    let routes = this.routes.get(host_header);
    let route = this._findBestRoute(routes);
    if (!route) {
        let routes = this.routes.get('*');
        route = this._findBestRoute(routes);
      if (!route) {
  ...
  maxSpread = 0;
  /**
   * Finds the least loaded route linear scale
   * @return {Object}
   */
  _findBestRoute(routes) {
    routes = [routes].flat(Infinity).filter(route => route);
    if(routes.length === 0) return null;
    let route = null;
    console.log(this.scale)
    const unusedRoutes = routes.filter(route => !this.load.find(usedRoute => usedRoute.endpoint === route));
    if (unusedRoutes.length > 0) {
        route = unusedRoutes[0];
        this.load.push({ endpoint: route, load: 1 });
    } else {
        const leastLoadedRoute = this.load.reduce((prev, curr) => (prev.load < curr.load) ? prev : curr, {  });
        const randomRoute = this.load[Math.floor(Math.random() * this.load.length)];
        route = this.scale === 'random' ? randomRoute.endpoint : leastLoadedRoute.endpoint;
        this.load = this.load.map(usedRoute => {
            if (usedRoute.endpoint === route) {
                usedRoute.load += 1;
            }
            return usedRoute;
        });
